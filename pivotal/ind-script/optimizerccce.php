...
MemberAction.plusAppSnsLogin = function () {
    var sSnsLoginType = location.search.split('sns_login_request=')[1] || '';
    var oXansMemberLoginTags = $('.xans-member-login').find('a');

    if (typeof sSnsLoginType !== 'undefined') {
        if (sSnsLoginType == 'facebook') {
            var sFacebookFuncRegex = /FBLogin\(\'member_form_+([0-9])+\'\)\;/;
...
$(document).ready(function(){
    ...
    try {
        if (window.location.hostname.substr(0, 2) == 'm.' || window.location.hostname.substr(0, 12) == 'mobile--shop' || window.location.hostname.substr(0, 11) == 'skin-mobile') {
            ...
            $('#order_id1').attr('placeholder', __('주문번호1'));
            $('#order_id1').attr('title', __('주문번호1'));

            $('#order_id2').attr('placeholder', __('주문번호2'));
            $('#order_id2').attr('title', __('주문번호2'));
 ...
 */
function resizeSnsLayer() 
{
    $(parent.top.document).contents().find('#ifrm_sns').height($('#mCafe24SnsAgree').height() + 2);
    $(parent.top.document).contents().find('#ifrm_sns').width($('#mCafe24SnsAgree').width());
}

// sns 연동후 회원가입 Layer 노출시 sns 연동으로 가져온 user 기본정보를 되도록 layer에 부려준다
function snsDefaultUserInfo(userInfo)
{
    ...
    try {
        if (userInfo.mobile1 && $("#sns_join #mobile1").length > 0) {
            $("#sns_join #mobile1 option").each(function() {
                if ($(this).val() == userInfo.mobile1) {
                    $(this).attr('selected', true);
                }
            });
        }
        if (userInfo.mobile2 && $("#sns_join #mobile2").length > 0) $("#sns_join #mobile2").val(userInfo.mobile2);
        if (userInfo.mobile3 && $("#sns_join #mobile3").length > 0) $("#sns_join #mobile3").val(userInfo.mobile3);
    } catch(e) {}
}
...
function snsAgreementLayer(sProviderTitle, userInfo)
{
    // 부모창에 중복으로 있으면 안되는 내용에 대해서 0 초과 엘리먼트들을 제거해준다
    $(parent.top.document).contents().find('div[id=mCafe24SnsAgree]:gt(0)').remove();
    ...
    $('#agreementClick').click(function(){
        // SNS 용 추가입력정보 필수체크 위해 수동 실행해준다 // essn_array 필수 input 배열 이름 정보 호출
        var result = true;
        var essn_array = [];
        set_essn();
        for (key in essn_array) {
            // sns 에서 사용하는 추가입력정보가 필수로 설정되어진 경우만 실행한다
            if (jQuery.inArray(essn_array[key], ['is_birthday', 'region', 'sex', 'phone', 'cell']) >= 0) {
                result = obj_checkessn.switch_action(essn_array[key]);
...
var sTopProviderCode = '';
var sTopProviderTitle = '';
var sTopOAuth2EncData = '';
var oTopUserInfo = '';

function openAgreementLayer(sProviderCode, oAuth2EncData, sProviderTitle)
{
    var oAuth2EncData = decodeURIComponent(oAuth2EncData);

    top.sTopProviderCode = sProviderCode;
    top.sTopProviderTitle = sProviderTitle;
...
function snsJoin()
{
    if (top.sTopProviderCode == 'Facebook') {
        getCurrentUserInfo(top.oTopUserInfo, false);
    } else {
        // 공통 input 값들에 대해 배열 변수에 담아준다
        sOauth2EncDataArray = snsOauth2EncDataSet(top.oTopUserInfo);
...
function openAgreementLayerStep2(oAuth2EncData)
{

    var sProviderCode = top.sTopProviderCode;
    var sProviderTitle = top.sTopProviderTitle;

    // sns 를 통한 로그인 시도시 처리
    if ($('#mCafe24SnsAgree') && $('#joinForm')) {

        // sns AgreementLayer 에서 네이버,페이스북 공통 처리되는 기능 펑션
        snsAgreementLayer(sProviderTitle);

        // sns 유저 기본 정보 노출
        var output = oAuth2EncData;
        if ( AuthSSLManager.isError(output) == true ) {
            alert(output);
            return;
        }
        output = decodeURIComponent(output);
        var userInfo = AuthSSLManager.unserialize(output);
        userInfo = JSON.parse(userInfo.sEncryptInfo);

        top.oTopUserInfo = userInfo;
        snsDefaultUserInfo(userInfo.aUserInfo);
        top.sTopOAuth2EncData = oAuth2EncData;

    } else {
        $('#agreementClick').click(function(){AgreementLayer.EncryptSubmit()});
    }
    $(parent.top.document).contents().find("input[name='sOauth2EncData']").val(oAuth2EncData);
...
{
    // FaceBook 용 셋팅
    aOauth2EncDataArray['aUserInfo']['accessToken'] = $("input[name='accessToken']").val();
    aOauth2EncDataArray['aUserInfo']['fb_id'] = $("input[name='fb_id']").val();
    aOauth2EncDataArray['aUserInfo']['fb_email'] = $("input[name='fb_email']").val();
    aOauth2EncDataArray['aUserInfo']['fb_name'] = $("input[name='fb_name']").val();
...
FwValidator.Handler.setExtensionFilter('isOrderid', function(Response) {
    ...
    sFieldName = Response.elmCurrField.attr('name');
    
    if ((aMatch = sFieldName.match(/[^0-9]*/)) == null) return;
    
    $('input[name^="' + aMatch[0] + '"]').each(function() {
        sValue += this.value;
    });
    
    /* 2.0 마이그레이션 주문번호 체계로 인한 주석처리
    if (sValue.match(/^(19|20)\d\d(0[1-9]|1[012])(0[1-9]|[12][0-9]|3[01])([0-9]{7})$/) != null) {
        bResult = this.parent.CODE_SUCCESS;
    }
    */
    
    var check = this.parent.Verify.isNumber(sValue);
    if (check === true) {
        bResult = this.parent.CODE_SUCCESS;
    }
...
//에러메세지 등록
FwValidator.Handler.setFilterErrorMsg('isOrderid', __('올바른 주문번호가 아닙니다'));
