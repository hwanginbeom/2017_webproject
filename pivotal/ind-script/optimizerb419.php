...
var BOARD_READ = {
    ...
    article_delete : function(sFormName, iBoardNo)
    {
        ...
        $('#bdf_action_type').val('delete');
        $('#'+sFormName).attr('action', $('#bdf_del_url').val()).submit();
    },
    ...
    article_modify : function(sFormName, iBoardNo)
    {
        ...
        $('#bdf_action_type').val('modify');
        $('#'+sFormName).attr('action', $('#bdf_modify_url').val()).submit();
    },
     ...
     * @returns {Boolean}
     */
    board_mileage : function(user_id, tmp, board_no, bbs_no)
    {
        if (!user_id) {
            alert(__('적립금바로주기는 회원에게만 가능합니다.'));
            return false;
        }

        href = getMultiShopUrl('/admin/php/c/member_admin_d_f.php?user_id=') + user_id + '&mode=3&comment_tmp=' + encodeURIComponent(tmp) + '&board_no=' + board_no + '&bbs_no=' + bbs_no;

        window.open(href, '', 'toolbar=no location=no scrollbars=yes resizable=yes width=680 height=780');
    },
     ...
     * @returns {Boolean}
     */
    board_read_mileage_frame : function(mile_again, member_id, board_no, product_name, bbs_no, no, currency)
    {
        if (isSubmitted == true) {
        ...
        var msg = sprintf('%s을 지급하시겠습니까?', (mile_money+currency));
        if ( confirm(msg) ) {
            if ( mile_again == 'T' ) {
                if ( confirm("이미 동일 상품명으로 지급된 적립금이 있습니다.\n그래도 지급하시겠습니까?") ) {
                    BOARD_READ.setMemberMileage(aParams);
                }
            } else {
                BOARD_READ.setMemberMileage(aParams);
            }
     ...
     * @param aParams 파라미터
     */
    setMemberMileage : function(aParams)
    {
        isSubmitted = true;
        $.post(getMultiShopUrl("/admin/php/c/member_admin_pop03_hidden.php"), aParams, function(req) {
            isSubmitted = false;
            alert(decodeURIComponent(req));
        }, 'text');
    },
     ...
     * 회원정보 보기 (관리자전용)
     * @param sMallId 몰아이디
     * @param sMemberId 회원아이디
     */
    member_info : function(sMallId, sMemberId)
    {
        if (sMemberId == '') {
            alert(__('잘못 된 접근입니다.'));
            return false;
        ...
        BOARD.OpenWindow(getMultiShopUrl('http://' + sMallId + '.cafe24.com/admin/php/c/member_admin_d_f.php?user_id=')+sMemberId, 'member_info', '700', '780','yes');
    },
    ...
    /**
     * 관리자 인증 게시글 게시 (관리자전용)
     * @param string action exec경로
     * @param int iNo 게시글번호
     */
    board_post : function(action, iNo)
    {
        var aData = {
                'no' : iNo
        }
        
        $.post(action, aData, function(req) {
            if (req.failed) {
                alert(req.msg);
                return;
            } else {
...
$(document).ready(function(){
    var obj = $(':input[name="comment_password"]');
...
var BOARD_COMMENT = {
    ...
    comment_insert : function(action_url)
    {
        ...
        if (result.passed) {
            ...
            if (BOARD_COMMENT.is_comment_submitting === false) {
                ...
                //SSL 처리
                var aEleId = [$("#commentWriteForm #comment_name"),
                              $("#commentWriteForm #comment_password")];
    ...
    comment_insert_ajax : function(output)
    {
        ...
        //암호화된 필드 내 값 제거
        $("#commentWriteForm #comment_name").val('');
        $("#commentWriteForm #comment_password").val('');
        ...
        $.post(BOARD_COMMENT.comment_insert_url, $.param(formData), function(req) {
            if (!req.failed) {
                self.location.reload();
            } else {
                BOARD_COMMENT.is_comment_submitting = false;
    ...
    comment_reply_insert : function(action_url)
    {
        ...
        if (result.passed) {
            ...
            if (BOARD_COMMENT.is_comment_reply_submitting === false) {
                ...
                //SSL 처리
                var aEleId = [$("#commentReplyWriteForm #comment_name"),
                              $("#commentReplyWriteForm #comment_password")];
    ...
    comment_reply_insert_ajax : function(output)
    {
        ...
        var form = $('#commentReplyWriteForm');
        
        //암호화된 필드 내 값 제거
        var sTempCommentName = '';
        sTempCommentName = $("#commentReplyWriteForm #comment_name").val();

        $("#commentReplyWriteForm #comment_name").val('');
        $("#commentReplyWriteForm #comment_password").val('');
        ...
        $.post(this.comment_reply_insert_url, $.param(formData), function(req) {
            if (!req.failed) {
                self.location.reload();
            } else {
                BOARD_COMMENT.is_comment_reply_submitting = false;
                form.children("input[name='"+AuthSSL.sEncryptId+"']").remove();
                $("#commentReplyWriteForm #comment_name").val(sTempCommentName);
                alert(decodeURIComponent(req.msg));
                return false;
            }
        },'json');  
    ...
    commentGet : function(iNo, iCommentNo)
    {
        ...
        if (form.comment_password !== '') {
            $("#comment_password").bind('keypress',function(e) {
                 if (e.keyCode==13) return false;
            });
        }

        $.get(action_url, aData, function(req) {
            if (req.failed == false) {
                var rData = req.data;
    ...
    comment_reply_write_new : function (iCommentNo, e)
    {
        ...
        if ($form.css('display') == 'none') {
            ...
            if ( $(e).parent().find('#commentReplyWriteForm').length < 1 ) {
                $p.after($form);
            }
        } else {
            $form.hide();
    ...
    {
        var $p = $(e).parent();
        var $form = $("#commentForm").css('display', 'none');
        $p.after($form);

        this.hide_secret_comment_form('commentSecretForm');

        // 비밀댓글인 경우 비밀번호 확인
        if ($('#comment_secret_link'+iCommentNo).length > 0) {
    ...
    {
        var $p = $(e).parent().parent();
        var sCommentName = $("#commentForm").get(0).comment_name.value;
        ...
        this.commentGet(iNo, iCommentNo);
        $("#commentForm").get(0).comment_name.value = sCommentName;

        $p.css('display', 'none');
        $p.siblings().eq(1).css('display', 'none');
    },
    ...
    comment_delete_opdiary : function (iBoardNo, iNo, iCommentNo, sMemberId)
    {
        ...
        $.post('/exec/front/Board/CommentDel/'+iBoardNo, aData, function(req) {
            if (!req.failed) {
                self.location.reload();
            } else {
                alert(req.msg);
    ...
    comment_update_ok : function(fname)
    {
        ...
        if (result.passed) {
            if (BOARD_COMMENT.is_comment_update_submitting === false) {
                ...
                //SSL 처리
                var aEleId = [$("#commentForm #comment_password")];
    ...
    comment_update_ajax : function(output)
    {
        ...
        //암호화된 필드 내 값 제거
        $("#commentForm #comment_password").val('');
        ...
        $.post(form.attr("action"), $.param(formData), function(req) {
            if (!req.failed) {
                self.location.reload();
            } else {
                BOARD_COMMENT.is_comment_update_submitting = false;
    ...
    comment_cancel_ok_new : function(fname)
    {
        ...
        $form.siblings().eq(1).css('display', '');
        $form.siblings().eq(2).css('display', '');
    },
    ...
    comment_byte : function(sFormName)
    {
        ...
        if (sFormName == 'commentForm') {
            ...
            BOARD_COMMENT.limitWord($('#'+content).get(0), comment_size, sFormName);
        }
    ...
    show_secret_comment_form : function(sFormName, iCommentNo, bAfterFilter)
    {
        ...
        if ($form.length > 0) {
            // 폼 리셋
            $form[0].reset();
            $('#comment_no', $form).val(iCommentNo);
            $form.removeClass('displaynone').css('display', 'block');
            $('#comment_secret_link' + iCommentNo).parent().after($form);
            if ($form.secure_password !== '') {
                $("#secure_password", $form).bind('keypress',function(e) {
                     if (e.keyCode==13) return false;
                });
            }
        }
    ...
    get_comment_callback : function(sFormName, aData)
    {
        ...
        $('#comment_modify', $form).val(aData['comment']);
        $('#member_id', $form).val(aData['member_id']);
        $('#comment_no', $form).val(aData['comment_no']);
        $('#comment_password', $form).val('');
        if ($('[name=secure]', $form).length > 0 && aData['secret_flag'] == 'T') {
            $('[name=secure]', $form).attr('checked', 'checked');
        }
        BOARD_COMMENT.comment_byte(sFormName);
        $form.css('display', 'block');
        $('#commentReplyWriteForm').css('display', 'none');
    },
     ...
     * 코멘트 쓰기 게시물에 비밀번호 세팅
     */
    setCmtPwd : function(sStr) {
        sStr = decodeURIComponent(sStr).split('').reverse().join('');
        $('input[name="comment_password"]', '.xans-board-commentwrite').val(sStr);
    },
