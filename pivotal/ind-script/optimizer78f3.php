...
$(document).ready(function()
{
    $('.btn_recent_del').click(function()
    {
        if (confirm(__('정말 삭제하시겠습니까?'))) {
            var nameOfCookie = "recent_plist";
            if (EC_SDE_SHOP_NUM > 1) {
                nameOfCookie = nameOfCookie + EC_SDE_SHOP_NUM;
            }
            $.cookie(nameOfCookie , EC_Recentlist.getRecentRemove(EC_Recentlist.getRecentData(), $(this).attr('rel')), {
                'path' : '/',
                'expires' : 365
            });
    ...
    /**
     * 사용자측 js(/js/module/myshop/ui.js) 에서
     * "옵션보기"버튼에 대한 show/hide 제어가 없어서 빈 버튼이 보여지고 있음.
     * 옵션이 보여져야하는 버튼만 보여지게 하고 나머지는 hide
     * ==> http://jira.simplexi.com/browse/SUB-9129
     */
    $('.show_opt').each(function() {
        if ($(this.parentNode.parentNode).find('ul').length > 0) $(this).show();
        else                                                     $(this).hide();
    });

    EC_Recentlist.setOptionLord();

    // 옵션변경
    $('select[id^="product_option_id"]').change(function(){
        if ($(this).attr('option_code')  != undefined) {
            var iProductCountNum = parseInt($(this).attr('class').replace('ProductOption',''));
            if ( Boolean($(this).attr('required')) === false && $(this).val() === '**' ){
                Olnk.getCheckValue($(this).val(),$(this));
                return false;
            }

            if ( EC_Recentlist.aOptionOlinkData[iProductCountNum]  === false ) {
                Olnk.getCheckValue($(this).val(),$(this));
            }
        }
...
var EC_Recentlist = {
    ...
    {
        if (EC_SDE_SHOP_NUM > 1) {
            var nameOfCookie = "recent_plist"+EC_SDE_SHOP_NUM+"=";
        } else {
            var nameOfCookie = "recent_plist=";
        }
        var x = 0;
     ...
     * 최근본상품값 삭제된후 저장될값 리턴
     */
    getRecentRemove: function(sPull, iPrd)
    {
        var aPull = sPull.split('|');
        var aReturn = new Array();

        $.each(aPull, function(iKey, sVal) {
            if (iPrd != sVal) {
                aReturn[iKey] = sVal;
            }
        ...
        if (EC_Recentlist.getIsLocalStorageAble() === true) {
           this.setProductRecentInfo(iPrd);
        }
     ...
     * 세트상품은 최근본상품에서 처리 안함.
     */
    getSetProduct : function()
    {
        alert(__('세트상품은 상품상세보기에서 구매하실 수 있습니다.'));
    },
    /*
     *  최근본 상품 옵션의 순차로딩
     */
    setOptionLord : function() {
        var iNum = 0;
        var aProductNum = new Array();
        var iProductNum = 1;
        var iCheckNum = 1;
        var bAllSelected = true;
        $(".xans-product-listitem>.xans-record-").each(function(){
            var aItemList = $(this).find('.ProductOption'+iNum);
            if ( aItemList.attr('option_code') != undefined ) {
                iProductNum =  1;
                iCheckNum = 1;
                bAllSelected = true;
                aItemList.each(function(){
                    var sProductId = 'product_option_id_' + iNum + '_' + iProductNum;
                    if ( iCheckNum > 1) {
                        bAllSelected = false;
                    }
                    if (iCheckNum === 1 && Boolean($(this).attr('required')) === false ) {
                        iCheckNum = 1;
                    } else {
                        iCheckNum++;
                    }
                ...
                EC_Recentlist.aOptionOlinkData[iNum] =  bAllSelected;
            }
            iNum++;
    ...
    setProductRecentInfo : function(iProductNo) {
        ...
        var oJsonData = sessionStorage.getItem(sStorageKey);
        if ( oJsonData === null ) {
            return;
        }
        var aNewStorageDataList = [];
        ...
        for (var iKey in aStorageData) {
            ...
            aNewStorageDataList.push(aStorageData[iKey]);
            iObjectKeyCount++;
        }

        sessionStorage.setItem(sStorageKey , $.toJSON(aNewStorageDataList));
    },
